...
func TestPrometheusHandlerCollector(t *testing.T) {
	...
	t.Run("empty name", func(t *testing.T) {
		collector, ok := handler.Collector("")
		assert.False(t, ok)
		assert.Nil(t, collector)
	})

	t.Run("non-existent metric", func(t *testing.T) {
		collector, ok := handler.Collector("non_existent")
		assert.False(t, ok)
		assert.Nil(t, collector)
	})
	...
	t.Run("existing metric", func(t *testing.T) {
		ctx := event.WithExporter(ctx, event.NewExporter(handler, eventtest.ExporterOptions()))
		c := event.NewCounter("existing_counter", &event.MetricOptions{
			Description: "Test existing counter",
		})
		c.Record(ctx, 1)

		collector, ok := handler.Collector("existing_counter")
		assert.True(t, ok)
		assert.NotNil(t, collector)
	})
}
